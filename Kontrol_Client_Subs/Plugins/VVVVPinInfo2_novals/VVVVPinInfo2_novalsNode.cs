...
{
	#region PluginInfo
	[PluginInfo(Name = "PinInfo2_novals", Category = "VVVV", Help = "Returns details about given Pins", Tags = "")]
	#endregion PluginInfo
	public class VVVVPinInfo2_novalsNode : IPluginEvaluate
	{
		#region fields & pins
...
//								FId[i] = node.ID;
							...
							if (pin.Type == "Enumeration") {
								string enumname = pin.SubType;
								//get the pins subtype
								enumname = enumname.Substring(enumname.IndexOf(',') + 2, enumname.LastIndexOf(',') - enumname.IndexOf(',') - 2);
								//cut out the part between the two commas (its the enum name)
								int enumcount = EnumManager.GetEnumEntryCount(enumname);
								//gets the enumentrycount. used to iterate through the entry indices 
								string enumentry = EnumManager.GetEnumEntry(enumname, 0);
								//get the first enum entry
								for (int j = 1; j < enumcount; j++) {
									// get the rest of the enum entries
									enumentry = enumentry + ", " + EnumManager.GetEnumEntry(enumname, j);
									//add the entries up to one string
								}
