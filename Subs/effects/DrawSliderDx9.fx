...
float values;
float4 cBG :color <String uiname="Color";> = { 1.0f,1.0f,1.0f,1.0f };
float4 cSlider :color <String uiname="Color2";> = { 0.0f,1.0f,1.0f,1.0f };
...
{
	float temp =(values>In.TexCd.x);
	
    float4 col = tex2D(Samp, In.TexCd)*cBG;
	col = (temp*cBG)+((1-temp)*cSlider);
    return col;
}
